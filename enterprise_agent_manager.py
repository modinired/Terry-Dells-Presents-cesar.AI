...
class EnterpriseAgentManager(LoggerMixin):
    ...
    async def get_active_agents(self) -> Dict[str, Any]:
        ...
        try:
            # Convert agent specs to active agent instances
            active_fleet = {}

            for agent_id, spec in self.agent_specs.items():
                # Create a mock agent instance representing the enterprise agent
                active_fleet[agent_id] = {
                    "agent_id": agent_id,
                    "name": spec.name,
                    "category": spec.category.value,
                    "capabilities": spec.capabilities,
            ...
            self.logger.info(f"Returning {len(active_fleet)} enterprise agents as active fleet")
            return active_fleet
